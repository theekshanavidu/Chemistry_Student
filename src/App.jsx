...
function AppInner() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#0b0f19] flex items-center justify-center transition-colors duration-200">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-semibold text-gray-500 dark:text-gray-400">SKCHEM.COM පූරණය වෙමින්...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0b0f19] text-gray-800 dark:text-gray-200 transition-colors duration-200">
      {authError && (
      ...
      <Routes>
        <Route path="/login" element={!user ? <Login onNavigate={(p) => navigate('/' + p)} theme={theme} toggleTheme={toggleTheme} /> : <Navigate to="/dashboard/home" replace />} />
        <Route path="/register" element={!user ? <Register onNavigate={(p) => navigate('/' + p)} theme={theme} toggleTheme={toggleTheme} /> : <Navigate to="/dashboard/home" replace />} />
        <Route
          path="/dashboard/*"
          element={
            user && studentData
              ? <StudentDashboard initialStudentData={studentData} onLogout={handleLogout} theme={theme} toggleTheme={toggleTheme} />
